...
def gen_action(state, meter_l):
    assert (
        ...
        len(meter_l) == (self.NUM_ACTIVE)
    ), "Please input `meter_l` with right format."
    assert (
        (type(state) == np.ndarray) and 
        (state.shape == (self.NUM_ACTIVE*6+self.NUM_FINISHED*7,))
    ...
    # pre = self.NUM_ACTIVE*6
    # for j in range(self.NUM_FINISHED):
    #     action = []
    #     action.extend(state[j*7+pre:(j+1)*7-2+pre])
    #     action.extend((meter_l[j+self.NUM_ACTIVE],))
    #     actions.append(action)
    print("Action: ")
    for a in actions:
        print(a)
...
def print_observation(obs):
    ...
    for i in range(self.NUM_ACTIVE):
        print(obs[6*i:6*(i+1)])
    bl = self.NUM_ACTIVE*6
    for i in range(self.NUM_FINISHED):
...
if __name__ == '__main__':
    ...
    obs = env.reset()
    for i in range(15):
        # meter_l = [random.choice([1, 10]) for _ in range(self.NUM_ACTIVE)]
        meter_l = [1 for _ in range(self.NUM_ACTIVE)]
