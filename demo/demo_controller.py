...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None, enable_meter=False):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def _packet_in_handler(self, ev):
        ...
        # ip route learning and install a flow entry 
        if pkt.get_protocols(ipv4.ipv4):
            ...
            if ip.proto == 6 or ip.proto == 17:
                if ip.proto == 6:
                    ...
                    tcp_src=tcpudp.src_port, tcp_dst=tcpudp.dst_port, 
                    ip_proto=ip.proto)
                if ip.proto == 17:
                    tcpudp = pkt.get_protocols(udp.udp)[0]
                    match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,
                    ipv4_src=ip.src, ipv4_dst=ip.dst, 
                    udp_src=tcpudp.src_port, udp_dst=tcpudp.dst_port, 
                    ip_proto=ip.proto)
                five_tuple = (ip.src, ip.dst, tcpudp.src_port, tcpudp.dst_port, ip.proto)
                print(five_tuple)
                if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                    self.add_flow(datapath, 1, match, actions, msg.buffer_id, enable_meter=True)
                    return
                else:
                    self.add_flow(datapath, 1, match, actions, enable_meter=True)
